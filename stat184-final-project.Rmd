...
# Renaming observations to prepare for join, changing into 3 groups; 1= some high school, 2= high school degree, 3= college degree
 primarymerged$Medu[primarymerged$Medu=="0"] <- "1"
 primarymerged$Medu[primarymerged$Medu=="2"] <- "1"
 primarymerged$Medu[primarymerged$Medu=="3"] <- "2"
 primarymerged$Medu[primarymerged$Medu=="4"] <- "2"
 
 primarymerged$Fedu[primarymerged$Fedu=="0"] <- "1"
 primarymerged$Fedu[primarymerged$Fedu=="2"] <- "1"
 primarymerged$Fedu[primarymerged$Fedu=="3"] <- "2"
 primarymerged$Fedu[primarymerged$Fedu=="4"] <- "3"
...
secondary %>%
  ...
  group_by(parentedu) %>%
  summarise(avgmath = mean(mathscore),
            avgreading = mean(readingscore),
            avgwriting = mean(writingscore))
...
mergedData %>%
  ...
  select(subject, sex, age, address, parentedu, Pstatus, studytime, famsup, higher, famrel, avgalc, grade, avgmath, avgreading, avgwriting)

# Create levels of "high" or "low" alcohol consumption

GlyphReady$avgalc <- ifelse(GlyphReady$avgalc >2.5, GlyphReady$avgalc <- "high", GlyphReady$avgalc <- "low")

# Rounding average scores
rounding(GlyphReady, 2)


# Change observation names of famsup

GlyphReady$famsup[GlyphReady$famsup=="yes"] <- "support"
GlyphReady$famsup[GlyphReady$famsup=="no"] <- "nosupport"
